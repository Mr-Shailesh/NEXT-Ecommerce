                  ...
                  <div className="mt-4 text-center md:text-left">
                    ...
                    <div className="mt-1">
                      ...
                      {products[item].color.includes("magenta") && (
                        <button className="border-2 border-gray-300 ml-1 bg-fuchsia-500 rounded-full w-6 h-6 focus:outline-none"></button>
                      )}
...
export const getServerSideProps = async (ctx) => {
  ...
  for (let item of products) {
    if (item.title in skin) {
      if (
        !skin[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        skin[item.title].color.push(item.color);
      }
      if (!skin[item.title].size.includes(item.size) && item.availableQty > 0) {
        skin[item.title].size.push(item.size);
      }
    } else {
      skin[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        skin[item.title].color = [item.color];
        skin[item.title].size = [item.size];
      }
    }
  }
  ...
  return {
    props: {
      products: JSON.parse(JSON.stringify(skin)),
    },
  };
};
